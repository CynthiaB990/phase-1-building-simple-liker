...
for (let i = 0; i < hearts.length; i++) {
  ...
  hearts[i].addEventListener("click", (e) => {
    e.target.textContent === FULL_HEART ?
      (e.target.textContent = EMPTY_HEART, e.target.classList.remove("activated-heart")) :
      (e.target.textContent = FULL_HEART, e.target.classList.add("activated-heart"));
    console.log(e.target.parentNode);
    // function to update the server << goes here
  })
}
